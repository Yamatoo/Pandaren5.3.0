...
void RealmList::UpdateRealm(uint32 id, const std::string& name, ACE_INET_Addr const& address, uint8 icon, RealmFlags flag, uint8 timezone, AccountTypes allowedSecurityLevel, float popu, uint32 build)
{
    ...
    address.addr_to_string(realm.RealmAddress, ACE_MAX_FULLY_QUALIFIED_NAME_LEN + 16);
...
void RealmList::UpdateRealms(bool init)
{
    ...
    if (result)
    {
        do
        {
            ...
            std::string realmAddress    = fields[2].GetString();
            ...
            ACE_INET_Addr addr(port, realmAddress.c_str(), AF_INET);
            ...
            if (init)
                TC_LOG_INFO(LOG_FILTER_AUTHSERVER, "Added realm \"%s\" at %s:%u.", name.c_str(), m_realms[name].RealmAddress, port);
        }
